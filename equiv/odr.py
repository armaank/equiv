...
def make_odr_figure() -> None:
    """Generate and save the ODR experiment figure (3 panels: simple, structured, noisy data).

    Fits a degree-150 polynomial with and without order-dependent regularization on each
    dataset and plots the predictions side by side. Saves the figure to ``odr_sample.png``.
    """
    degree = 150
    seed = 1111
    panels = [
    ...
    for ii, (data_type, title, bounds, y_limits, alpha_reg, n_samples) in enumerate(
        ...
        coeffs_reg, xmin_r, xmax_r = fit_polynomial(
            x, y, degree, alpha=alpha_reg, odr="exp_odr"
        )

        x_plot = jnp.linspace(bounds[0], bounds[1], 400)
        y_unreg = predict_polynomial(x_plot, coeffs_unreg, xmin_u, xmax_u)
        y_reg = predict_polynomial(x_plot, coeffs_reg, xmin_r, xmax_r)

        ax = axes[ii]
        ax.plot(x, y, "og", ms=4, markeredgecolor="black")
        ax.plot(x_plot, y_unreg, color="red", linewidth=2, label="Unregularized")
        ax.plot(
            x_plot,
            y_reg,
            color="blue",
    ...
    fig.tight_layout()
    logger.info("Saving figure to 'odr_sample.png'")
